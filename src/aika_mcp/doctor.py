...
from src.aika_mcp.host_configs import (
    DEFAULT_HOST,
    DEFAULT_SERVER_NAME,
    AikaMcpConfigError,
    build_mcp_config,
...
def run_mcp_doctor(
    *,
    ...
    try:
        ...
        checks.append(DoctorCheck("uv", STATUS_PASS, f"Using MCP command: {config['command']}"))
        ...
        checks.append(
            DoctorCheck(
                ...
                STATUS_FAIL,
                str(exc),
                "Install uv and retry, or run AIKA from an environment where uv is on PATH.",
            )
        )
